...
const quotes = {
  ...
  79: {
    file: 'airplane.wav',
    phrase: "Surely you can't be serious - I am serious. And don't call me Shirley",
...
function randomQuote(quotesVariants, myFilter = []) {
  ...
  if (Object.keys(quotesVariants).length > 1) {
    ...
    if (myFilter.length === 0) {
      ...
      return `<speak><audio src="https://iuriid.github.io/public/img/BestMovieQuotesWAVs/${
        ...
        randPair.movie
      }</audio><break time="3s"/>// ${prompt}</speak>`;
    }
    // We have a filter
  ...
  return `<speak><audio src="https://iuriid.github.io/public/img/BestMovieQuotesWAVs/${
    theOnlyQuote.file
  }">"${theOnlyQuote.phrase}" - ${theOnlyQuote.character} (${theOnlyQuote.actor}) - ${
    theOnlyQuote.movie
  }</audio><break time="3s"/>// ${prompt}</speak>`;
}
